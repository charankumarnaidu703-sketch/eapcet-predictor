...
export default function CollegeDetailClient({ collegeName, data }: CollegeDetailClientProps) {
  const router = useRouter();
  const c = data[0];
  const years    = [...new Set(data.map(r => r.year))].sort((a, b) => b - a);
  const branches = [...new Set(data.map(r => r.branch))].sort();
  const maxY = Math.max(...data.map(x => x.year));

  const [selBranch, setSelBranch] = useState(data.find(r => r.year === maxY)?.branch ?? data[0].branch);
  const [activeYear, setActiveYear] = useState(maxY);
          ...
          <div className="detail-hero-actions" style={{ marginTop: 32, display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <ShareButton 
              ...
              text={generateCollegeShareText(
                c.college_name,
                c.location,
                typeof window !== 'undefined' ? window.location.origin : 'https://ranksure.vercel.app'
              )} 
            />
          </div>
        </div>
